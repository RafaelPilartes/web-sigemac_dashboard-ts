...
import { TableRow } from '../../components/table/TableRowOpinion'
import { SelectCustom } from '../../components/selects/SelectCustom'
import { ModalEditOpinion } from '../../components/modal/opinion/ModalEditOpinion'
import { ModalCreateOpinion } from '../../components/modal/opinion/ModalCreateOpinion'
...
import { ModalSeeOpinion } from '../../components/modal/opinion/ModalSeeOpinion'

function Opinions() {
  const [rowsData, setRowsData] = useState<OpinionInterface[] | null>(null)
  ...
  const itemsBreadcrumbs = [
    ...
    { label: 'Opiniões', to: routsNameMain.admins },
    { label: 'Listagem' }
  ]
  ...
  function fetchData(limit: string) {
    ...
    // Get
    opinionViewModel.getAllOpinionData().then(response => {
      if (response.data.error) {
        showToastRight('error', response.data.msg as string)
      } else {
        const arrayData = response.data.data as unknown as OpinionInterface[]
        setTotalDocs(arrayData.length)
        console.log(arrayData)

        const listData = arrayData.slice(0, Number(limit))

        setRowsData(listData as OpinionInterface[])
      }
    })
  }
  ...
  async function searchDocs() {
    if (termForSearch == '') {
      ...
      opinionViewModel.getAllOpinionByTermData(termForSearch).then(response => {
        console.log(response)

        setRowsData(response.data.data as unknown as OpinionInterface[])
        console.log(response)
      })
    }
  }
  ...
  function handleDeleteRow(id: string) {
    swal({
      title: 'Tem certeza?',
      text: 'Uma vez excluído, você não poderá recuperar está opinion!',
      buttons: ['Cancelar', 'Confirmar'],
      icon: 'warning',
      dangerMode: true
    }).then(async willDelete => {
      if (willDelete) {
        await opinionViewModel.deleteOpinionData(id).then(response => {
          console.log(response)
